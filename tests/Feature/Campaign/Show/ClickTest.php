...
beforeEach(function () {
    ...
    $emailList = EmailList::factory()->state(['deleted_at' => null])->has(
        Subscriber::factory()
            ->count(3)
            ->sequence(
                ['name' => 'Joe Doe Test', 'email' => 'joe@email.com'],
                ['name' => 'Jane'],
                ['name' => 'Josh'],
            )
    )->create();
...
it('should list all campaign mails and show the clicks on each email', function () {
    get(route('campaigns.show', ['campaign' => $this->campaign, 'what' => 'clicked']))
        ...
        ->assertSeeTextInOrder([$this->mail1->subscriber->name, $this->mail1->clicks, $this->mail1->subscriber->email])
        ->assertSeeTextInOrder([$this->mail2->subscriber->name, $this->mail2->clicks, $this->mail2->subscriber->email])
        ->assertSeeTextInOrder([$this->mail3->subscriber->name, $this->mail3->clicks, $this->mail3->subscriber->email]);
...
it('should be possible to filter by email', function () {
    get(route('campaigns.show', ['campaign' => $this->campaign, 'what' => 'clicked', 'search' => 'joe@email.com']))
        ->assertViewHas('what', 'clicked')
        ->assertViewHas('query', function ($query) {
            expect($query)->toHaveCount(1);
